...
function Attack(first, second, x, y)
{
	this.first = first;
	this.first_x = first.attr("cx");
	this.first_y = first.attr("cy");
	this.second = second;
	this.second_x = second.attr("cx");
	this.second_y = second.attr("cy");
	this.x = x;
	this.y = y;
}

function AttackPosition(x, y)
{
	this.x = x;
	this.y = y;
}
...
{
	var visualize = true;
	var red_team = new Team(LEFT_MARGIN, SCREEN_HEIGHT / 4 * 2 + TOP_MARGIN, 6, 15, "red", "0-#a90329:30-#1f0001:100");
	var red_quadrant = new Raphael(red_team.color, "100%", "100%");
	var red_nodes;
	var blue_team = new Team(LEFT_MARGIN, SCREEN_HEIGHT / 4 + TOP_MARGIN, 6, 15, "blue", "0-#0455e0:30-#020087:100");
	var blue_quadrant = new Raphael(blue_team.color, "100%", "100%");
	var blue_nodes;
	var green_team = new Team(SCREEN_WIDTH / 4 + LEFT_MARGIN, TOP_MARGIN, 6, 15, "green", "0-#04e526:30-#00290c:100");
	var green_quadrant = new Raphael(green_team.color, "100%", "100%");
	var green_nodes;
	var yellow_team = new Team(SCREEN_WIDTH / 4 * 2 + LEFT_MARGIN, TOP_MARGIN, 6, 15, "yellow", "0-#f2f200:30-#292b00:100");
	var yellow_quadrant = new Raphael(yellow_team.color, "100%", "100%");
	var yellow_nodes;
	var purple_team = new Team(SCREEN_WIDTH / 4 * 3 + LEFT_MARGIN, SCREEN_HEIGHT / 4 + TOP_MARGIN, 6, 15, "purple", "0-#ca00d1:30-#260029:100");
	var purple_quadrant = new Raphael(purple_team.color, "100%", "100%");
	var purple_nodes;
	var orange_team = new Team(SCREEN_WIDTH / 4 * 3 + LEFT_MARGIN, SCREEN_HEIGHT / 4 * 2 + TOP_MARGIN, 6, 15, "orange", "0-#ef6700:30-#3b0700:100");
	var orange_quadrant = new Raphael(orange_team.color, "100%", "100%");
	var orange_nodes;
	var other_team = new Team(SCREEN_WIDTH / 4 + LEFT_MARGIN, SCREEN_HEIGHT / 4 * 3 + TOP_MARGIN, 8, 31, "other", "0-#acacac:30-#292929:100");
	var other_quadrant = new Raphael(other_team.color, "100%", "100%");
	var other_nodes;

	var info = new Raphael("info", "100%", "100%");
	var title = "2013 Cyber Storm";
	var cs = info.print(SCREEN_WIDTH / 4 / 2 - title.length * 25 / 2 + LEFT_MARGIN, SCREEN_HEIGHT / 4 / 2 - 50 + TOP_MARGIN, title, info.getFont("agency fb"), 65).attr({fill: "gray", stroke: "white"});
	info.print(SCREEN_WIDTH / 4 * 0.5 - red_team.color.length * 10 / 2 + LEFT_MARGIN, SCREEN_HEIGHT / 4 * 2 + TOP_MARGIN, red_team.color.toUpperCase(), info.getFont("agency fb"), 16).attr({fill: "#a90329"});
	info.print(SCREEN_WIDTH / 4 * 0.5 - blue_team.color.length * 10 / 2 + LEFT_MARGIN, SCREEN_HEIGHT / 4 + TOP_MARGIN, blue_team.color.toUpperCase(), info.getFont("agency fb"), 16).attr({fill: "#0455e0"});
	info.print(SCREEN_WIDTH / 4 * 1.5 - green_team.color.length * 10 / 2 + LEFT_MARGIN, TOP_MARGIN, green_team.color.toUpperCase(), info.getFont("agency fb"), 16).attr({fill: "#04e526"});
	info.print(SCREEN_WIDTH / 4 * 2.5 - yellow_team.color.length * 10 / 2 + LEFT_MARGIN, TOP_MARGIN, yellow_team.color.toUpperCase(), info.getFont("agency fb"), 16).attr({fill: "#f2f200"});
	info.print(SCREEN_WIDTH / 4 * 3.5 - purple_team.color.length * 10 / 2 + LEFT_MARGIN, SCREEN_HEIGHT / 4 + TOP_MARGIN, purple_team.color.toUpperCase(), info.getFont("agency fb"), 16).attr({fill: "#ca00d1"});
	info.print(SCREEN_WIDTH / 4 * 3.5 - orange_team.color.length * 10 / 2 + LEFT_MARGIN, SCREEN_HEIGHT / 4 * 2 + TOP_MARGIN, orange_team.color.toUpperCase(), info.getFont("agency fb"), 16).attr({fill: "#ef6700"});
	info.print(SCREEN_WIDTH / 4 * 2 - other_team.color.length * 10 / 2 + LEFT_MARGIN, SCREEN_HEIGHT / 4 * 3 + TOP_MARGIN, other_team.color.toUpperCase(), info.getFont("agency fb"), 16).attr({fill: "#acacac"});
	...
	var attack_pos = [];
	attack_pos.push(new AttackPosition(SCREEN_WIDTH / 4 * 1.33, SCREEN_HEIGHT / 4 * 1.25 * 4.5 / 5));
	attack_pos.push(new AttackPosition(SCREEN_WIDTH / 4 * 2, SCREEN_HEIGHT / 4 * 1.25 * 4.5 / 5));
	attack_pos.push(new AttackPosition(SCREEN_WIDTH / 4 * 2.66, SCREEN_HEIGHT / 4 * 1.25 * 4.5 / 5));
	attack_pos.push(new AttackPosition(SCREEN_WIDTH / 4 * 1.33, SCREEN_HEIGHT / 4 * 2 * 4.5 / 5));
	attack_pos.push(new AttackPosition(SCREEN_WIDTH / 4 * 2, SCREEN_HEIGHT / 4 * 2 * 4.5 / 5));
	attack_pos.push(new AttackPosition(SCREEN_WIDTH / 4 * 2.66, SCREEN_HEIGHT / 4 * 2 * 4.5 / 5));
	attack_pos.push(new AttackPosition(SCREEN_WIDTH / 4 * 1.33, SCREEN_HEIGHT / 4 * 2.75 * 4.5 / 5));
	attack_pos.push(new AttackPosition(SCREEN_WIDTH / 4 * 2, SCREEN_HEIGHT / 4 * 2.75 * 4.5 / 5));
	attack_pos.push(new AttackPosition(SCREEN_WIDTH / 4 * 2.66, SCREEN_HEIGHT / 4 * 2.75 * 4.5 / 5));
	...
	{
		attacks = [];
		attacks.push(new Attack(red_nodes[Math.floor(Math.random() * red_nodes.length - 1)], blue_nodes[Math.floor(Math.random() * blue_nodes.length - 1)], attack_pos[0].x, attack_pos[0].y));
		attacks.push(new Attack(green_nodes[Math.floor(Math.random() * green_nodes.length - 1)], orange_nodes[Math.floor(Math.random() * orange_nodes.length - 1)], attack_pos[1].x, attack_pos[1].y));
		attacks.push(new Attack(yellow_nodes[Math.floor(Math.random() * yellow_nodes.length - 1)], purple_nodes[Math.floor(Math.random() * purple_nodes.length - 1)], attack_pos[2].x, attack_pos[2].y));
		attacks.push(new Attack(red_nodes[Math.floor(Math.random() * red_nodes.length - 1)], other_nodes[Math.floor(Math.random() * other_nodes.length - 1)], attack_pos[3].x, attack_pos[3].y));
		attacks.push(new Attack(green_nodes[Math.floor(Math.random() * green_nodes.length - 1)], other_nodes[Math.floor(Math.random() * other_nodes.length - 1)], attack_pos[4].x, attack_pos[4].y));
		attacks.push(new Attack(purple_nodes[Math.floor(Math.random() * purple_nodes.length - 1)], orange_nodes[Math.floor(Math.random() * orange_nodes.length - 1)], attack_pos[5].x, attack_pos[5].y));
		attacks.push(new Attack(yellow_nodes[Math.floor(Math.random() * yellow_nodes.length - 1)], blue_nodes[Math.floor(Math.random() * blue_nodes.length - 1)], attack_pos[6].x, attack_pos[6].y));
		attacks.push(new Attack(other_nodes[Math.floor(Math.random() * other_nodes.length - 1)], green_nodes[Math.floor(Math.random() * green_nodes.length - 1)], attack_pos[7].x, attack_pos[7].y));
		attacks.push(new Attack(red_nodes[Math.floor(Math.random() * red_nodes.length - 1)], yellow_nodes[Math.floor(Math.random() * yellow_nodes.length - 1)], attack_pos[8].x, attack_pos[8].y));
	}

	function showAttacks()
	{
		attacks[0].first.animate({cx: attack_pos[0].x - ATTACK_OFFSET, cy: attack_pos[0].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[0].x + "," + attack_pos[0].y + "r-360"}, 1500, function()
			{
				repeat++;
				if (repeat == ATTACK_REPEAT)
					attacks[0].first.animate({cx: attacks[0].first_x, cy: attacks[0].first_y, r: NODE_RADIUS}, 1000, "back-in", function()
					{
						setTimeout(refresh, 1000);
					});
			});
			attacks[0].first.animate(anim.repeat(ATTACK_REPEAT));
		});
		attacks[0].second.animate({cx: attack_pos[0].x + ATTACK_OFFSET, cy: attack_pos[0].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[0].x + "," + attack_pos[0].y + "r-360"}, 1500, function()
			{
			...
			attacks[0].second.animate(anim.repeat(ATTACK_REPEAT));
		});
		attacks[1].first.animate({cx: attack_pos[1].x - ATTACK_OFFSET, cy: attack_pos[1].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[1].x + "," + attack_pos[1].y + "r-360"}, 1500, function()
			{
				repeat++;
				if (repeat == ATTACK_REPEAT)
			...
			attacks[1].first.animate(anim.repeat(ATTACK_REPEAT));
		});
		attacks[1].second.animate({cx: attack_pos[1].x + ATTACK_OFFSET, cy: attack_pos[1].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[1].x + "," + attack_pos[1].y + "r-360"}, 1500, function()
			{
			...
			attacks[1].second.animate(anim.repeat(ATTACK_REPEAT));
		});
		attacks[2].first.animate({cx: attack_pos[2].x - ATTACK_OFFSET, cy: attack_pos[2].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[2].x + "," + attack_pos[2].y + "r-360"}, 1500, function()
			{
				repeat++;
				if (repeat == ATTACK_REPEAT)
			...
			attacks[2].first.animate(anim.repeat(ATTACK_REPEAT));
		});
		attacks[2].second.animate({cx: attack_pos[2].x + ATTACK_OFFSET, cy: attack_pos[2].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[2].x + "," + attack_pos[2].y + "r-360"}, 1500, function()
			{
			...
			attacks[2].second.animate(anim.repeat(ATTACK_REPEAT));
		});
		attacks[3].first.animate({cx: attack_pos[3].x - ATTACK_OFFSET, cy: attack_pos[3].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[3].x + "," + attack_pos[3].y + "r-360"}, 1500, function()
			{
				repeat++;
				if (repeat == ATTACK_REPEAT)
			...
			attacks[3].first.animate(anim.repeat(ATTACK_REPEAT));
		});
		attacks[3].second.animate({cx: attack_pos[3].x + ATTACK_OFFSET, cy: attack_pos[3].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[3].x + "," + attack_pos[3].y + "r-360"}, 1500, function()
			{
			...
			attacks[3].second.animate(anim.repeat(ATTACK_REPEAT));
		});
		attacks[4].first.animate({cx: attack_pos[4].x - ATTACK_OFFSET, cy: attack_pos[4].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[4].x + "," + attack_pos[4].y + "r-360"}, 1500, function()
			{
				repeat++;
				if (repeat == ATTACK_REPEAT)
			...
			attacks[4].first.animate(anim.repeat(ATTACK_REPEAT));
		});
		attacks[4].second.animate({cx: attack_pos[4].x + ATTACK_OFFSET, cy: attack_pos[4].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[4].x + "," + attack_pos[4].y + "r-360"}, 1500, function()
			{
			...
			attacks[4].second.animate(anim.repeat(ATTACK_REPEAT));
		});
		attacks[5].first.animate({cx: attack_pos[5].x - ATTACK_OFFSET, cy: attack_pos[5].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[5].x + "," + attack_pos[5].y + "r-360"}, 1500, function()
			{
				repeat++;
				if (repeat == ATTACK_REPEAT)
			...
			attacks[5].first.animate(anim.repeat(ATTACK_REPEAT));
		});
		attacks[5].second.animate({cx: attack_pos[5].x + ATTACK_OFFSET, cy: attack_pos[5].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[5].x + "," + attack_pos[5].y + "r-360"}, 1500, function()
			{
			...
			attacks[5].second.animate(anim.repeat(ATTACK_REPEAT));
		});
		attacks[6].first.animate({cx: attack_pos[6].x - ATTACK_OFFSET, cy: attack_pos[6].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[6].x + "," + attack_pos[6].y + "r-360"}, 1500, function()
			{
				repeat++;
				if (repeat == ATTACK_REPEAT)
			...
			attacks[6].first.animate(anim.repeat(ATTACK_REPEAT));
		});
		attacks[6].second.animate({cx: attack_pos[6].x + ATTACK_OFFSET, cy: attack_pos[6].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[6].x + "," + attack_pos[6].y + "r-360"}, 1500, function()
			{
			...
			attacks[6].second.animate(anim.repeat(ATTACK_REPEAT));
		});
		attacks[7].first.animate({cx: attack_pos[7].x - ATTACK_OFFSET, cy: attack_pos[7].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[7].x + "," + attack_pos[7].y + "r-360"}, 1500, function()
			{
				repeat++;
				if (repeat == ATTACK_REPEAT)
			...
			attacks[7].first.animate(anim.repeat(ATTACK_REPEAT));
		});
		attacks[7].second.animate({cx: attack_pos[7].x + ATTACK_OFFSET, cy: attack_pos[7].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[7].x + "," + attack_pos[7].y + "r-360"}, 1500, function()
			{
			...
			attacks[7].second.animate(anim.repeat(ATTACK_REPEAT));
		});
		attacks[8].first.animate({cx: attack_pos[8].x - ATTACK_OFFSET, cy: attack_pos[8].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[8].x + "," + attack_pos[8].y + "r-360"}, 1500, function()
			{
				repeat++;
				if (repeat == ATTACK_REPEAT)
			...
			attacks[8].first.animate(anim.repeat(ATTACK_REPEAT));
		});
		attacks[8].second.animate({cx: attack_pos[8].x + ATTACK_OFFSET, cy: attack_pos[8].y, r: ATTACK_RADIUS}, 2000, "back-out", function()
		{
			var repeat = 0;
			var anim = Raphael.animation({transform: "...r360," + attack_pos[8].x + "," + attack_pos[8].y + "r-360"}, 1500, function()
			{
			...
			attacks[8].second.animate(anim.repeat(ATTACK_REPEAT));
		});
	}
});
...
function showNodes(team, quadrant)
{
	...
	var cols = team.cols;
	var num_nodes = max_nodes;//Math.floor(Math.random() * max_nodes + 10);
	var row = 0;
	var col = 0;

	for (var i=0; i<num_nodes; i++)
	{
		var node = quadrant.circle(team.x + (col + 1) * NODE_OFFSET, team.y + (row + 1) * NODE_OFFSET, NODE_RADIUS);
	
		node.attr({fill: team.gradient, stroke: "#494949"});
		nodes.push(node);
